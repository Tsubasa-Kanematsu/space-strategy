...
function buildPhaseFlow(opts: PhaseFlowOpts): AnalysisFlowStep[] {
  ...
  return [
    ...
    { id: df,  order: 1,  label: '分散飛行経路解析',       kind: 'normal',   position: { x: X(1), y: -95 },  nextStepIds: [ol, sh, pec, di, gn, lsb, rf], ...base },
    { id: ol,  order: 2,  label: '軌道上寿命解析',         kind: 'normal',   position: { x: X(2), y: -285 }, nextStepIds: [ab], ...base },
    { id: ab,  order: 3,  label: '溶融解析',              kind: 'normal',   position: { x: X(3), y: -285 }, nextStepIds: [J], ...base },
    { id: sh,  order: 4,  label: '海上船舶危険解析',       kind: 'normal',   position: { x: X(2), y: -190 }, nextStepIds: [J], ...base },
    { id: pec, order: 5,  label: '地上落下傷害予測数解析', kind: 'normal',   position: { x: X(2), y: -95 },  nextStepIds: [J], ...base },
    { id: di,  order: 6,  label: '投棄物落下域解析',       kind: 'normal',   position: { x: X(2), y: 0 },    nextStepIds: [J], ...base },
    { id: gn,  order: 7,  label: 'GNSS可視解析/COLA',      kind: 'normal',   position: { x: X(2), y: 95 },   nextStepIds: [J], ...base },
    { id: lsb, order: 8,  label: '射場内建屋危険解析',     kind: 'normal',   position: { x: X(2), y: 190 },  nextStepIds: [J], ...base },
    { id: rf,  order: 9,  label: 'RFリンク解析',           kind: 'normal',   position: { x: X(2), y: 285 },  nextStepIds: [J], ...base },
    { id: prr, order: 10, label: '経路回転率解析',         kind: 'normal',   position: { x: X(1), y: 400 },  nextStepIds: [ddf], ...base },
    { id: ddf, order: 11, label: '破片抗力落下予測域解析', kind: 'normal',   position: { x: X(2), y: 400 },  nextStepIds: [gi], ...base },
    { id: gi,  order: 12, label: 'ゲート侵犯可否',         kind: 'normal',   position: { x: X(3), y: 400 },  nextStepIds: [J], ...base },
